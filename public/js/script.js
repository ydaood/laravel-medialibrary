...
$(document).on('click', '#chooseItems', function (e) {
  selectFiles();
  localStorage.removeItem('items');
});
$(document).on('change', 'input[name="selectedItem[]"]', function (e) {
    //
    if($(this).is(':checked')){
        appendItem($(this));
    }else{
...
function removeItem(u){
    ...
    const index = items.indexOf(u.val());
    items.splice(index, 1);
    localStorage.items=JSON.stringify(items);
}
function appendItem(u){
...
function selectFiles(){
    var values=JSON.parse(localStorage.items);
    /*$("input[name='selectedItem[]']:checked").each(function(i){
          values[i] = $(this).val();
        });*/
    window.opener.setArray(values);
     if (window.opener) {
      window.close();
    }
}
